...
-   ou une **bibliographie annotée** : une énumération commentée d'oeuvres, sous la forme de liste ou sous la forme de paragraphes rédigés. Elle peut être présentée sous la forme de paragraphes qui se suivent avec un jeu par paragraphes ou par phrases de manière consécutive. Chaque élément est mentionné spécifiquement pour soutenir une attribution.

    ![Exemple de bibliographie annotée (Apocalypse World 1)](apocalypseWorld-ex.png){width="500"}

-   ou des **références dans le texte** : des mentions à des oeuvres dans un paragraphe de texte. Ces mentions sont insérées dans le discours pour l'illustrer ou pour donner des exemples, mais ces mentions ne sont pas listées systématiquement et pourquoi elles sont utilisées spécifiquement. Ou bien, il y a très peu de références dans le jeu et donc ce ne sont pas vraiment des listes/énumérations.\
    ![Exemple de référence dans le texte (Aquelarre 3)](aquelarre3.png){width="500"}
...
SELECT ?item ?itemLabel ?usesLabel ?NbOfRecords ?AtPages WHERE {
  ...
  # Find items where P2283 (uses) is either bibliography or annotated bib. or inline ref.
  ?item p:P2283 ?statement.
  ?statement ps:P2283 ?uses.
  FILTER(?uses IN (wd:Q4769616, wd:Q1631107, wd:Q137842886))
...
Le plus ancien jeu avec des références dans le texte est *OD&D* (1974) avec des attributions moyennement reconnaissante envers la campagne de Blackmoor de Dave Arneson, matrice du game design des jeux de rôle tels que nous les connaissons aujourd'hui.

Rapidement, les premiers jeux tels que *Tunnels & Trolls* ou *RuneQuest* ont cité D&D avant d'arrêter. Voir mon billet sur les [*Ceases & Desists*](https://jdr.hypotheses.org/1199) sur le sujet.

Le jeu le plus ancien avec une bibliographie est \[à déterminer\]. Avec une bibliographie annotée \[à déterminer\].

## Épigraphes

J'avais commencé à indexer la présence d'épigraphes dans les jeux de rôle sur table.

J'avais utilisé aussi la propriété uses avec comme valeur :
...
Exemple : [7th Sea (1st edition)](https://www.wikidata.org/wiki/Q110883344)
